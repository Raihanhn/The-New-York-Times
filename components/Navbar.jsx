...
export default function Navbar() {
  ...
  return (
    <header className="bg-white border-b border-gray-300 ">
      {/* First Layer: Menu left, Buttons right */}
      <div className="max-w-[1200px] mx-auto flex justify-between items-center p-2 ">
        {/* Left: Nav menu */}
        <nav className="hidden md:flex gap-4 font-medium text-black">
          {links.map((item, idx) => {
            const href = `/${
            ...
            return (
              <Link key={idx} href={href} className="relative pb-1 text-black ">
                {item}
                <span
        ...
        <div className="hidden md:flex gap-2">
          {/* 🔒 Don't render buttons until user fetch completes */}
          {!loading && (
            <>
              {user?.subscriptionStatus !== "active" && (
                <button
                  onClick={handleSubscribe}
                  className="px-4 py-2 text-xs bg-black text-white border-none cursor-pointer"
                >
              ...
              {!session ? (
                <button
                  ...
                  className="px-4 py-2 text-sm border border-black bg-white cursor-pointer"
                >
                  Login with Google
                </button>
              ) : (
      ...
      {/* Second Layer: Logo centered */}
      <div className=" flex justify-center md:justify-center mb-2">
        <svg
          viewBox="0 0 184 25"
          fill="#000"
          aria-hidden="true"
          className="w-40 md:w-28 lg:w-[680px] h-auto"
        >
      ...
      {/* Mobile Menu Button */}
      {/* Mobile wrapper */}
      <div className="md:hidden absolute mt-[-45px] flex items-center">
        {/* Hamburger button */}
        <button
        ...
        {mobileMenuOpen && !loading && (
          <div className="absolute top-full left-0 w-[97vw] bg-white shadow-md flex flex-col p-4 gap-3 z-50">
            {links.map((item, idx) => {
              const href = `/${
                item.toLowerCase() === "home" ? "" : item.toLowerCase()
              }`;
